...
def find_galaxy_coords(galaxy_num, gal_np):
    for row_idx in range(gal_np.shape[0]):
        for col_idx in range(gal_np.shape[1]):
            if gal_np[row_idx][col_idx] == galaxy_num:
...
def main():
    ...
    print(gal_np_conv)
    gal_perms = [i for i in list(combinations(range(1,num_gal), 2))]
    print(len(gal_perms))

    # Find empty rows, cols index
    empty_cols_idx = find_empty_cols(gal_np_conv)
    empty_rows_idx = find_empty_rows(gal_np_conv)

    sum_path_between_galaxies = 0

    coord_list_num1 = [ find_galaxy_coords(each_pair[0], gal_np_conv) for each_pair in gal_perms]
    coord_list_num2 = [ find_galaxy_coords(each_pair[1], gal_np_conv) for each_pair in gal_perms]

    for each_pair in range(len(gal_perms)):
        # print(each_pair)
        # Find the coordinates of each pair
    
        x1, y1 = coord_list_num1[each_pair]
        x2, y2 = coord_list_num2[each_pair]
        # print(x1, y1)
        # print(x2, y2)
...
# x1, y1 = find_galaxy_coords(each_pair[0], gal_np_conv)
# x2, y2 = find_galaxy_coords(each_pair[1], gal_np_conv)
